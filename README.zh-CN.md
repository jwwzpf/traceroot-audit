...
**面向本地 agent 的独立运行时审计产品。**

TraceRoot Audit 会陪着 OpenClaw、MCP 以及其他本地 agent runtime 一起跑：当它们开始尝试高风险动作时，TraceRoot 会及时提醒，并把整条审计轨迹留在本地，方便你之后回看。
...
这条本地审计时间线现在也会自动保持在一个轻量范围内，所以就算你把陪跑模式开很久，`events.jsonl` 也不会在后台无限长大。
...
当 target、已批准边界、提醒方式这三件事都已经记住以后，`doctor --watch` 现在会直接续上上次那套陪跑，不再重新展开整套 Doctor 和补丁包生成流程；只有你明确加上 `--reconfigure` 时，TraceRoot 才会重新带你走一遍设置。
...
如果 TraceRoot 能看见同一个动作从“开始尝试”一路走到“已经完成”或“没有完成”，这条过程现在也会完整保留在时间线里，所以 `logs --today` 会更像真实发生过的事情，而不是一串底层事件。

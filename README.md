...
When all three pieces are already known — the target, the approved boundary, and the reminder preference — `doctor --watch` can now jump back in with a single confirmation instead of rebuilding the whole setup flow.

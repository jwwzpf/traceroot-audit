...
That local audit timeline now also stays bounded automatically, so long-running watch sessions do not keep growing an unbounded `events.jsonl` file in the background.
...
When all three pieces are already known — the target, the approved boundary, and the reminder preference — `doctor --watch` now resumes that watch session automatically instead of walking you through setup again. If you want to start fresh, add `--reconfigure`.
